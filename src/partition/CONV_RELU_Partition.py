...
def CONV_RELU_Partition(partition:int, model:onnx.ModelProto, conv_node_name:str, relu_node_name:str,concat_node_name:str) -> onnx.ModelProto:
    ...
    for i in range(partition):
        ...
        name = conv_weight.name + f"_split_{i}_weight"
    ...
    if len(conv_node.input) == 3:
        ...
        bias_Name = []
        for i in range(partition):
            name = conv_bias.name + f"_split_{i}_bias"
            shape = bias.shape
            minn = sum(number[0:i])
    ...
    for i in range(partition):
        name = conv_output.name + f"_split_{i}_output"
    ...
    for i in range(partition):   
        name = relu_output.name + f"_split_{i}_output"
    ...
    model.graph.node.insert(relu_index + partition , concat)

    model = onnx.shape_inference.infer_shapes(model)
    onnx.checker.check_model(model)
    

    return model, convNameList, reluNameList